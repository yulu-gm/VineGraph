...
CLI 仍可直接运行旧版 `.yaml` 示例，例如 `examples/project-task-loop.yaml`。在产品工作台中，graph asset 使用 `.vg.yaml` / `.vg.yml`；旧版 `.yaml` 示例可以继续通过 CLI 执行，或导入 / 复制为 `.vg.yaml` graph asset 后在工作台中使用。
