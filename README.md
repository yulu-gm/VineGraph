...
`start-tauri.bat` 会从项目根目录完成启动准备：检查 Node.js / npm / Microsoft C++ Build Tools / Rust / Cargo，缺失时优先通过 `winget` 安装 Node.js LTS、Visual Studio Build Tools 和 Rustup，随后执行 `npm.cmd install`，构建 debug 版 Tauri 客户端，启动本地 server，再打开桌面版。首次启动可能会花更久。
...
没有设置时会尝试使用 PATH 或常见 npm 全局安装位置里的 `codex.cmd` / `claude.cmd`。
