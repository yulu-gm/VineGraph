...
- 读取 `examples/*.vg.yaml` / `examples/*.vg.yml` 里的单层 graph 配置。
- 产品工作台把 `.vg.yaml` / `.vg.yml` 作为 VineGraph graph asset 格式，在打开的项目目录中自动发现这些资产。
- 支持 `execute` 节点，业务 backend 只保留 `codex`、`claude`；`internal` 仅用于流程终点和流转占位。
...
`npm.cmd run example` 会执行 `examples/simple-smoke.vg.yaml`，跑一个最小 Codex 只读 smoke graph。成功后会在 `.agentgraph/runs/` 下生成本次运行记录。
...
```bash
npm start -- examples/simple-smoke.vg.yaml
```

Windows:

```powershell
npm.cmd start -- examples/simple-smoke.vg.yaml
```

CLI 和产品工作台统一使用 `.vg.yaml` / `.vg.yml` graph asset。`examples/` 下不再保留普通 `.yaml` 示例。
...
在产品工作台中，先打开一个项目目录。左侧 Repository / Graph Assets 会发现该目录下的 `.vg.yaml` / `.vg.yml` graph asset，选择 asset 后画布、可编辑 Inspector 和运行面板会绑定到同一个 graph。底部 workspace bar 显示并切换本次运行的 workspace target，运行按钮会把当前 graph 对选中的 workspace target 执行。
...
VineGraph graph asset 保存为 `.vg.yaml` 或 `.vg.yml`。产品工作台和示例目录都只使用这个格式。
...
edges:
  - from: graph.start
    to: smoke_step.inputs.trigger

  - from: smoke_step.outputs.done
...
```bash
npm start -- examples/read-only-backend-loop-dispatch-smoke.vg.yaml
```
...
src/
  ...
  execute-runner.ts     codex / claude 执行器，internal 流程动作
...
- graph execute backend 不再提供 shell / git / human 这类非大模型执行节点。
