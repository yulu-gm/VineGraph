...
## Graph Canvas 连线与端口规则

后续所有节点端口和连线渲染必须固定使用以下规则，不允许在单个功能里另起一套连线风格：

- 节点输入 pin 固定在左侧，输出 pin 固定在右侧，整体语义保持左进右出。
- 连接线必须使用水平 / 垂直的正交折线，不使用斜线或自由曲线。
- 每条连接线从输出 pin 离开后，必须先保持一段固定水平距离；不要让线一出来就转弯。
- 每条连接线进入输入 pin 前，也必须保留一段固定水平距离；输入端和输出端的切线方向都必须朝正右方。
- 所有转角使用同一个固定圆角半径；如果相邻线段太短，圆角半径必须自动缩小，不能让圆角反向或穿越端点。
- Controller 输出必须按输出端口颜色渲染，输出列表、输出 pin、连线和箭头颜色要保持一致。
- 回环线必须从节点上方或下方绕行，按 lane 分层，尽量避开跨越范围内的节点，不要直接穿过节点主体。
- 连线可以在视觉层级上高于节点以保证可见，但不能阻挡节点点击、选择和 Inspector 操作。
...
- [ ] 节点端口和连线遵守“左进右出、正交折线、固定圆角、固定 pin 水平段、回环上下绕行”的 Graph Canvas 规则。
